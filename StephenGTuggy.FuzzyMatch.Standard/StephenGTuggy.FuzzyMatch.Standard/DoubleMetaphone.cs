 ...
 * 	1.1.0 - sgt - Fork
 * 	1.2.0 - sgt - Minor Update
 *
 * This implementation, and optimizations, Copyright (C) 2003, Adam J. Nelson; and
...
namespace nullpointer.Metaphone
{
    ...
    /// Further modified by Stephen Tuggy (sgt@StephenGTuggy.com).
    /// </summary>
	public class DoubleMetaphone
    {
        // The length of the metaphone keys produced.  4 is sweet spot.
        // ** SGT ** Construct full-length key to store in database or whatever, then shorten it later, in other code.
        public const int METAPHONE_KEY_LENGTH = 50; //4;

        private readonly StringBuilder _PrimaryKey;
        private readonly StringBuilder _AlternateKey;
		...
		public String AlternateKey {
            get {
                return _HasAlternate ? _AlternateKeyString : null;
            }
        }
		...
		static public void CalcDoubleMetaphone(String word, ref String primaryKey, ref String alternateKey) {
        ...
        private bool IsWordSlavoGermanic() {
            if ((_Word.IndexOf("W") != -1) ||
                (_Word.IndexOf("K") != -1) ||
                (_Word.IndexOf("CZ") != -1) ||
                (_Word.IndexOf("WITZ") != -1))
                return true;
